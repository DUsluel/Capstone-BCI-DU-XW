...
global obs_side_data; obs_side_data = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function on_close(varargin)% should not be closed before stopping the stream i.e. colData usage        
        try
            ...
            delete(th);
            delete(h_fig);
            [filtRes, powRes] = filtering(EEGdata(:,1:8),250);
            ...
            Mdl = fitcsvm(powdata(:,1:8),obs_side_data);
    ...
    function on_timer(varargin)
        ...
        try
            if obs_side == -1
                ...
                [samp,timestamps] = inlet.pull_sample();
                samp
                if isequal(samp,[-1.875 -1.875 -1.875 -1.875 -1.875 -1.875 -1.875 -1.875]*1.0e+5)
                    disp('returned')
                    return;
                else
                    [obs_h,obs_pos,obs_side] = setObsPos(obs_h,obs_pos,obs_side);
                    obs_side_data = [obs_side_data;obs_side];
                    drawnow;
                end
            %end
