function streamFiltering(varargin)
...
opts = arg_define(varargin, ...
    ...
    arg({'Model','model'},[],[],'Contains the trained classifier model'),... % created afterwards for data streaming in to matlab workspace
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    obs_side = -1; mdl = evalin('base','Mdl');mdl
    start(th);    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function on_close(varargin)% should not be closed before stopping the stream i.e. colData usage        
        try
            ...
            assignin('base','predict_data_tst',predicdata);
        catch
        end
    end    
    ...
    function on_timer(varargin)
        ...
        try
            ...
            if obs_side ~= -1
                ...
                if length(filtTbl) <= 55               
                    filtTbl = [filtTbl; [samp obs_side ms*60+ss]];
                else
                    filtTbl = [filtTbl; [samp obs_side ms*60+ss]];                    
                    filt = filtering(filtTbl(:,1:8),250);
                    P = predict(mdl,filt);
                    moveObjPos(round(mean(P)),obj_h);
                    predicdata = [predicdata;P];
                    disp('Filtered')
                    filtdata = [filtdata; filtTbl];
                    powdata = [powdata; [filt obs_side*ones(4,1)]];                    
                    filtTbl = [];                    
                end                             
            end
